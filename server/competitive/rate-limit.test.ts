...
describe("competitive rate limit", () => {
  it("allows a burst of five and then returns a retry window", () => {
    for (let index = 0; index < 5; index += 1) {
      expect(consumeCompetitiveRateLimit("player", 0).remaining).toBe(4 - index);
    }
  ...
  it("refills tokens over time and isolates players", () => {
    for (let index = 0; index < 5; index += 1) consumeCompetitiveRateLimit("player", 0);
    expect(consumeCompetitiveRateLimit("other-player", 0).remaining).toBe(4);
