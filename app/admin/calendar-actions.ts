...
function commandMessage(code: string) {
  switch (code) {
    case "season_not_found": return "La temporada ya no existe. Recarga el portal.";
    case "season_not_active": return "La temporada debe estar activa.";
    case "room_not_found": return "La sala ya no está disponible.";
    case "content_not_found": return "El contenido ya no existe. Recarga el portal.";
    case "content_not_published": return "Selecciona contenido publicado.";
    case "unsupported_content": return "Solo se puede programar Flash publicado compatible.";
    case "invalid_schedule_dates": return "La ventana no es válida o queda fuera de la temporada.";
    case "schedule_not_found": return "La publicación ya no existe. Recarga el portal.";
    case "schedule_already_open": return "La publicación ya ha comenzado y no se puede reprogramar.";
    case "schedule_not_editable": return "Solo se pueden reprogramar publicaciones futuras.";
    case "schedule_number_conflict": return "Ese número ya está ocupado en la temporada.";
    case "schedule_overlap": return "La ventana se solapa con otra publicación.";
    case "schedule_conflict": return "La publicación cambió en otra pestaña. Recarga antes de guardar.";
    case "idempotency_conflict": return "Esta operación ya se usó con otros datos. Recarga el formulario.";
    default: return "No se ha podido guardar la programación. Inténtalo de nuevo.";
  }
}
...
function parseCommon(formData: FormData) {
  ...
  if (!uuidPattern.test(challengeVersionId)) fieldErrors.challengeVersionId = "Selecciona contenido publicado.";
  if (!Number.isSafeInteger(number) || number <= 0) fieldErrors.number = "Usa un número entero positivo.";
  ...
  if (idempotencyKey.length < 8 || idempotencyKey.length > 160) fieldErrors.form = "Recarga el formulario para preparar la operación.";
  if (reason.length === 0 || reason.length > 500) fieldErrors.reason = "Introduce un motivo de hasta 500 caracteres.";
  return { idempotencyKey, reason, number, challengeVersionId, opensAtLocal, closesAtLocal, fieldErrors };
}
...
export async function createScheduledChallenge(
  ...
  try {
    ...
    if (!uuidPattern.test(seasonId)) parsed.fieldErrors.seasonId = "Selecciona una temporada activa.";
    const room = findSeasonRoom(access.context, seasonId);
    const season = room?.seasons.find((candidate) => candidate.seasonId === seasonId);
    if (!room || !season || season.status !== "active") parsed.fieldErrors.seasonId = "Selecciona una temporada activa.";
    const window = room ? parseWindow(parsed, room.timeZone) : null;
    if (!window || Object.keys(parsed.fieldErrors).length > 0) return validationState(parsed.fieldErrors);
    try {
...
export async function updateScheduledChallenge(
  ...
  try {
    ...
    const context = await getSuperadminCalendarContext();
    const entry = context.entries.find((candidate) => candidate.scheduledChallengeId === scheduledChallengeId);
    const parsed = parseCommon(formData);
    if (!uuidPattern.test(scheduledChallengeId) || !entry) parsed.fieldErrors.scheduledChallengeId = "Selecciona una publicación futura.";
    if (!expectedUpdatedAt || Number.isNaN(Date.parse(expectedUpdatedAt))) parsed.fieldErrors.expectedUpdatedAt = "La publicación está desactualizada. Recarga el portal.";
    const room = entry ? access.context.rooms.find((candidate) => candidate.roomId === entry.roomId) : null;
    const window = room ? parseWindow(parsed, room.timeZone) : null;
    if (!window || Object.keys(parsed.fieldErrors).length > 0) return validationState(parsed.fieldErrors);
    try {
