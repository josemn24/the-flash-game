...
export async function POST(
  ...
  try {
    ...
    const result = await commandsFor(identity).submitWordSearchSelection({
      ...
      startCell: requireCell(body, "startCell"),
      endCell: requireCell(body, "endCell"),
      clientTimeUsedMs: optionalClientTime(body) as DurationMs | undefined,
    });
    return responseFor(result, 200, requestId, "competitive.attempt.word_search_select", startedAt);
  } catch (error) {
    return errorResponse(error, requestId, "competitive.attempt.word_search_select", startedAt);
  }
}
