-- S05 competitive read boundary. Alphabet releases only letter metadata and
-- public short-text payloads while active; solutions are terminal-only reads.

create function public.get_my_alphabet_challenge(target_room_slug text, target_publication_id uuid)
returns table (
  room_id uuid,
  room_slug text,
  room_title text,
...
create function public.get_my_alphabet_result(target_attempt_id uuid)
returns table (
  attempt_id uuid,
...
alter function public.get_my_alphabet_challenge(text, uuid) owner to postgres;
alter function public.get_my_alphabet_result(uuid) owner to postgres;
revoke all on function public.get_my_alphabet_challenge(text, uuid), public.get_my_alphabet_result(uuid)
  from public, anon, service_role;
grant execute on function public.get_my_alphabet_challenge(text, uuid), public.get_my_alphabet_result(uuid)
  to authenticated;
