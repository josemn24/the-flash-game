-- S15 — Pyramid projections. Live reads expose level metadata only; prepared
-- question payloads and terminal solutions stay behind their dedicated paths.
...
revoke all on function public.get_my_pyramid_challenge(text, uuid), public.get_my_pyramid_result(uuid)
  from public, anon, service_role;
grant execute on function public.get_my_pyramid_challenge(text, uuid), public.get_my_pyramid_result(uuid)
  to authenticated;
