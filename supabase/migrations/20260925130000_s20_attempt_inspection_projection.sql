begin;
-- S20 follow-up: align the persisted inspection RPC with the portal projection.
...
revoke all on function public.get_superadmin_attempt_inspection(uuid, uuid, uuid) from public, anon, service_role;
grant execute on function public.get_superadmin_attempt_inspection(uuid, uuid, uuid) to authenticated;
commit;
