...
-- S15 format slice 1/4: true-false. Reuse the published question validator.
...
begin
  if expected_type not in ('true-false', 'ordering', 'classification', 'logic-matrix') then
    return false;
  end if;
...
begin
  ...
  if question.type in ('true-false', 'ordering', 'classification', 'logic-matrix') then
