-- Keep the migration in sync with supabase/schemas/57_superadmin_reads.sql.
-- The dashboard has a separate read boundary so /admin does not need to load
-- editorial documents, private solutions, the full question library, or the
-- complete calendar.
...
begin
  ...
  return jsonb_build_object(
    ...
    'metrics', jsonb_build_object(
      'activeRooms', (select count(*) from public.rooms room where room.status = 'active'),
      'activeSeasons', (
      ...
      'editorialDrafts', (
        select count(*) from private.challenge_versions version
      ...
      'upcomingChallenges', (
        ...
        where room.status = 'active' and season.status = 'active'
          and version.mode = 'flash' and version.status = 'published'
          and schedule.status in ('scheduled', 'open') and schedule.closes_at >= now()
      )
    ),
    'rooms', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'seasonCount', (select count(*) from public.seasons season_count where season_count.room_id = room.id),
        'activeSeason', (
          select jsonb_build_object('title', season.title, 'startsAt', season.starts_at, 'endsAt', season.ends_at)
      ...
      from public.rooms room where room.status = 'active'
    ), '[]'::jsonb),
    'upcomingChallenges', coalesce((
      ...
      from (
        select schedule.id as scheduled_challenge_id, room.id as room_id, room.title as room_title,
          season.id as season_id, season.title as season_title, version.id as challenge_version_id,
          version.title as challenge_title, schedule.number as challenge_number, schedule.status as challenge_status,
          schedule.opens_at, schedule.closes_at, room.time_zone
        from public.scheduled_challenges schedule
        join public.seasons season on season.id = schedule.season_id
        join public.rooms room on room.id = season.room_id
        join private.challenge_versions version on version.id = schedule.challenge_version_id
