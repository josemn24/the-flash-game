-- E02: preserve an open Logic-code interaction during recovery, matching E01.
CREATE OR REPLACE FUNCTION private.prepare_interaction(input jsonb)
RETURNS jsonb
...
begin
  ...
  if exists (
    ...
    where x.attempt_id = (input->>'attemptId')::uuid and x.ended_at is null and cv.mode = 'flash'
      and q.type not in ('mini-wordle', 'logic-code')
  ) then
...
CREATE OR REPLACE FUNCTION private.recover_attempt(input jsonb)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO ''
AS $function$
declare
...
REVOKE ALL ON FUNCTION private.prepare_interaction(jsonb), private.recover_attempt(jsonb)
  FROM public, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION private.prepare_interaction(jsonb), private.recover_attempt(jsonb) TO service_role;
