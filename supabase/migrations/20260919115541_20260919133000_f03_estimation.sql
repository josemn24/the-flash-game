...
CREATE OR REPLACE FUNCTION private.editorial_has_secret_key(value jsonb)
 RETURNS boolean
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO ''
...
$function$;
CREATE OR REPLACE FUNCTION private.is_supported_flash_question(target_question uuid)
 RETURNS boolean
 LANGUAGE plpgsql
...
$function$;
CREATE OR REPLACE FUNCTION private.read_competitive_question_asset(input jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
...
$function$;
CREATE OR REPLACE FUNCTION private.validate_flash_editorial_document(document jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
begin
  ...
  loop
    ...
    if jsonb_typeof(question->'slug') is distinct from 'string'
      ...
      or question->>'type' not in ('multiple-choice', 'estimation', 'mini-wordle', 'logic-code', 'progressive-clues', 'matching', 'progressive-image', 'queens', 'true-false', 'odd-one-out', 'ordering', 'anagram', 'classification')
      or (question->>'type' not in ('progressive-image', 'multiple-choice', 'estimation') and question->'payloadSchemaVersion' <> '1'::jsonb)
      or (question->>'type' in ('progressive-image', 'multiple-choice') and question->'payloadSchemaVersion' not in ('1'::jsonb, '2'::jsonb))
      or (question->>'type' = 'estimation' and question->'payloadSchemaVersion' <> '2'::jsonb)
...
$function$;
REVOKE SELECT ON private.mini_wordle_dictionary_words FROM service_role;
REVOKE SELECT ON private.mini_wordle_guess_events FROM service_role;
