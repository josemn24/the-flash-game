-- S08 — Create private room and bootstrap its initial memberships.
create or replace function private.room_slug_base(value text) returns text
...
create or replace function private.create_room_command(input jsonb) returns jsonb
...
begin
  ...
  if not exists (select 1 from pg_catalog.pg_timezone_names where name = time_zone_value) then
  ...
  safe_input := jsonb_build_object(
    'idempotencyKey', key, 'title', title_value, 'description', description_value,
    'timeZone', time_zone_value, 'ownerEmail', owner_email_value,
    'initialMembers', normalized_members, 'reason', reason_value
  );
  ...
  from auth.users auth_user join public.players p on p.auth_user_id = auth_user.id
  where lower(auth_user.email) = owner_email_value and p.status = 'active';
  if not found then raise exception 'owner_not_found' using errcode = '22023'; end if;
  ...
  loop
    slug_value := case when slug_suffix = 1 then slug_base
      else left(slug_base, 64 - char_length(slug_suffix::text) - 1) || '-' || slug_suffix::text end;
  ...
  result := jsonb_build_object(
    'roomId', member_id, 'slug', slug_value, 'title', title_value,
  ...
  audit_payload := jsonb_build_object(
    'roomId', member_id, 'slug', slug_value, 'title', title_value,
    'timeZone', time_zone_value, 'ownerPlayerId', owner_id,
  ...
  values (actor, 'create_room', 'room', member_id, reason_value, key, null, audit_payload);
...
create or replace function public.lookup_superadmin_players(target_emails text[])
...
create or replace function public.create_superadmin_room(input jsonb) returns jsonb
...
revoke all on function private.room_slug_base(text), private.create_room_command(jsonb) from public, anon, authenticated, service_role;
revoke all on function public.lookup_superadmin_players(text[]), public.create_superadmin_room(jsonb) from public, anon, service_role;
revoke execute on function public.lookup_superadmin_players(text[]), public.create_superadmin_room(jsonb) from service_role;
grant execute on function public.lookup_superadmin_players(text[]), public.create_superadmin_room(jsonb) to authenticated;
