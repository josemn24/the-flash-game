-- S15 — persisted Pyramid levels, authoritative outcomes and own terminal review.

begin;

set local check_function_bodies = off;

-- private.is_valid_pyramid_level_config
...
-- public.get_room_introduction
create or replace function public.get_room_introduction(target_room_slug text, target_publication_id uuid)
returns table (
  room_id             uuid,
  room_slug           text,
  room_title          text,
...
-- public.get_my_pyramid_challenge
create function public.get_my_pyramid_challenge(target_room_slug text, target_publication_id uuid)
returns table (
  room_id uuid,
  room_slug text,
  room_title text,
...
-- public.get_my_pyramid_result
create function public.get_my_pyramid_result(target_attempt_id uuid)
returns table (
  attempt_id uuid,
...
-- public.get_superadmin_challenge_catalog
...
-- public.get_superadmin_challenge_detail
...
-- private.assert_supported_calendar_content
create or replace function private.assert_supported_calendar_content(version_id uuid) returns void
language plpgsql stable security definer set search_path = '' as $$
declare
  version_status text;
  version_mode text;
...
-- public.get_superadmin_calendar_context
create or replace function public.get_superadmin_calendar_context() returns jsonb
language plpgsql stable security definer set search_path = '' as $$
declare
  actor uuid := (select private.current_player_id());
begin
...
-- public.get_superadmin_room_calendar_context
create or replace function public.get_superadmin_room_calendar_context(target_room_id uuid) returns jsonb
language plpgsql stable security definer set search_path = '' as $$
declare
  actor uuid := (select private.current_player_id());
begin
...
-- public.get_room_calendar
create or replace function public.get_room_calendar(target_room_slug text)
returns table (
  room_id uuid,
  room_slug text,
  room_title text,
...
-- private.validate_flash_editorial_document
...
-- private.publish_flash_command
create or replace function private.publish_flash_command(input jsonb) returns jsonb
...
-- private.read_superadmin_editorial_context
create or replace function private.read_superadmin_editorial_context() returns jsonb
language plpgsql stable security definer set search_path = '' as $$
declare
  actor uuid := (select private.current_player_id());
begin
...
revoke all on function private.is_valid_pyramid_level_config(jsonb) from public, anon, authenticated, service_role;

alter function public.get_my_pyramid_challenge(text, uuid) owner to postgres;

alter function public.get_my_pyramid_result(uuid) owner to postgres;

revoke all on function public.get_my_pyramid_challenge(text, uuid), public.get_my_pyramid_result(uuid) from public, anon, service_role;

grant execute on function public.get_my_pyramid_challenge(text, uuid), public.get_my_pyramid_result(uuid) to authenticated;
