const namespace = "the-flash-game:s04";

const domainIds = {
  room: "s04-room-main",
  season: "s04-season-main",
  challenge: "s04-challenge",
  challengeVersion: "s04-challenge-version",
  questionOne: "s04-question-one",
  questionTwo: "s04-question-two",
  questionVersionOne: "s04-question-version-one",
  questionVersionTwo: "s04-question-version-two",
  challengeItemOne: "s04-challenge-item-one",
  challengeItemTwo: "s04-challenge-item-two",
  publication: "s04-publication",
};
...
export const scenario = {
  ...
  buildDomainSql({ accounts, sqlString, sqlUuid }) {
    ...
    const questionOne = publicQuestion("s04-question-one", "¿Cuál es la capital de Portugal?", [
      "Lisboa",
      "Oporto",
      "Braga",
      "Coímbra",
...
values (${sqlUuid(domainIds.room)}, 's04-main', 'Sala competitiva S04', 'Recuperación y abandono explícito');
...
values
  ...
  (${sqlUuid(domainIds.room)}, ${sqlString(carol)}, 'member', 'active', ${sqlString(dateStart)});
insert into public.seasons (id, room_id, title, status, starts_at, ends_at)
values (${sqlUuid(domainIds.season)}, ${sqlUuid(domainIds.room)}, 'Temporada S04', 'active',
  ${sqlString(dateStart)}, ${sqlString(dateEnd)});
insert into private.question_definitions (id, slug, created_by_player_id)
values
  (${sqlUuid(domainIds.questionOne)}, 's04-question-one', ${sqlString(alice)}),
  (${sqlUuid(domainIds.questionTwo)}, 's04-question-two', ${sqlString(alice)});
...
values
  (${sqlUuid(domainIds.questionVersionOne)}, ${sqlString(
    JSON.stringify({
      ...
      explanation: "S04_EXPLANATION_ONE: Lisboa es la capital de Portugal.",
    }),
  )}),
  (${sqlUuid(domainIds.questionVersionTwo)}, ${sqlString(
    JSON.stringify({
      correctAnswer: "Marte",
      explanation: "S04_EXPLANATION_TWO: Marte recibe el nombre de planeta rojo.",
    }),
  )});
...
values (${sqlUuid(domainIds.challenge)}, 's04-flash-recovery', ${sqlString(alice)});
...
values (${sqlUuid(domainIds.challengeVersion)}, ${sqlUuid(domainIds.challenge)}, 1, 1, 'draft', 'flash',
  'Flash recuperación S04', 'Dos preguntas de 50 puntos',
  'Recupera una partida sin reiniciar su estado.', 100, '{}', ${sqlString(alice)}, null);
...
`;
  ...
  manifest({ stableId }) {
    return {
      room: { id: stableId(domainIds.room), slug: "s04-main" },
