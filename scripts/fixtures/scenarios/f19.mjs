const namespace = "the-flash-game:f19";

const domainIds = {
  room: "f19-room-main",
  season: "f19-season-main",
  challenge: "f19-challenge",
  challengeVersion: "f19-challenge-version",
  questionOne: "f19-question-one",
  questionTwo: "f19-question-two",
  questionVersionOne: "f19-question-version-one",
  questionVersionTwo: "f19-question-version-two",
  challengeItemOne: "f19-challenge-item-one",
  challengeItemTwo: "f19-challenge-item-two",
  publication: "f19-publication",
};
...
export const scenario = {
  ...
  namespace,
  users: [
    { label: "superadmin", displayName: "Operador F19" },
    { label: "alice", displayName: "Alice F19" },
    { label: "charlie", displayName: "Charlie F19" },
    { label: "bob", displayName: "Bob F19" },
  ],
  ...
  buildDomainSql({ accounts, sqlString, sqlUuid }) {
    ...
    const words = { top: "YOGUI", bottom: "REUMA", left: "PONER", right: "QUEMA" };
...
values (${sqlUuid(domainIds.room)}, 'f19-main', 'Sala competitiva F19', 'Flash mixto con Word-hashtag');
...
values (${sqlUuid(domainIds.season)}, ${sqlUuid(domainIds.room)}, 'Temporada F19', 'active', ${sqlString(dateStart)}, ${sqlString(dateEnd)});
insert into private.question_definitions (id, slug, created_by_player_id)
values
  (${sqlUuid(domainIds.questionOne)}, 'f19-question-one', ${sqlString(accounts.alice.playerId)}),
  (${sqlUuid(domainIds.questionTwo)}, 'f19-question-two', ${sqlString(accounts.alice.playerId)});
...
values (${sqlUuid(domainIds.challenge)}, 'f19-flash-word-hashtag', ${sqlString(accounts.alice.playerId)});
...
values (${sqlUuid(domainIds.challengeVersion)}, ${sqlUuid(domainIds.challenge)}, 1, 1, 'draft', 'flash',
  'Flash F19 Word-hashtag', 'Elección múltiple + Word-hashtag',
  'Intercambios validados en servidor y solución privada.', 100, '{}',
...
`;
  ...
  manifest({ stableId }) {
    return {
      room: { id: stableId(domainIds.room), slug: "f19-main" },
