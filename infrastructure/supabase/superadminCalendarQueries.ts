...
function isCalendarEntry(value: unknown): value is SuperadminCalendarEntry {
  ...
  return (
    typeof value.scheduledChallengeId === "string" && uuidPattern.test(value.scheduledChallengeId) &&
    typeof value.roomId === "string" && uuidPattern.test(value.roomId) &&
    typeof value.roomSlug === "string" && typeof value.roomTitle === "string" &&
    typeof value.timeZone === "string" && isValidTimeZone(value.timeZone) &&
    typeof value.seasonId === "string" && uuidPattern.test(value.seasonId) &&
    typeof value.seasonTitle === "string" && typeof value.seasonStatus === "string" && seasonStatuses.has(value.seasonStatus) &&
    typeof value.challengeVersionId === "string" && uuidPattern.test(value.challengeVersionId) &&
    typeof value.challengeSlug === "string" && typeof value.versionNumber === "number" && Number.isSafeInteger(value.versionNumber) && value.versionNumber > 0 &&
    typeof value.challengeTitle === "string" && typeof value.challengeSubtitle === "string" && value.mode === "flash" &&
    typeof value.number === "number" && Number.isSafeInteger(value.number) && value.number > 0 &&
    typeof value.status === "string" && statuses.has(value.status) && iso(value.opensAt) && iso(value.closesAt) && iso(value.updatedAt)
  );
}
...
function isCalendarResult(value: unknown): value is Omit<SuperadminCalendarCommandResult, "source"> {
  return isRecord(value) &&
    ...
    iso(value.opensAt) && iso(value.closesAt) && iso(value.updatedAt);
}

function isTickResult(value: unknown): value is Omit<import("@/application/ports/superadmin-calendar-commands").RunCalendarTickResult, "source"> {
  return isRecord(value) && typeof value.runId === "string" && iso(value.evaluatedAt) &&
    typeof value.opened === "number" && Number.isSafeInteger(value.opened) && value.opened >= 0 &&
    typeof value.closed === "number" && Number.isSafeInteger(value.closed) && value.closed >= 0 &&
    typeof value.finishedSeasons === "number" && Number.isSafeInteger(value.finishedSeasons) && value.finishedSeasons >= 0;
}

function commandCode(error: { code?: string; message?: string }) {
  const message = error.message ?? "";
  const known = [
    "not_authorized", "invalid_command", "season_not_found", "season_not_active", "room_not_found",
    "content_not_found", "content_not_published", "unsupported_content", "invalid_schedule_dates",
    "schedule_not_found", "schedule_not_editable", "schedule_already_open", "schedule_number_conflict",
    "schedule_overlap", "schedule_conflict", "calendar_tick_unauthorized", "calendar_tick_invalid",
...
{
  async getContext(): Promise<SuperadminCalendarContext> {
    const supabase = await createClient();
    const { data, error } = await supabase.rpc("get_superadmin_calendar_context");
    if (error) {
      if (error.code === "42501" || error.message.includes("not_authorized")) {
        throw new SuperadminAccessDeniedError();
      }
    ...
    if (!isCalendarContext(data)) throw new Error("Supabase calendar read returned an invalid context payload.");
  ...
  async runCalendarTick() {
    ...
    try {
      ...
      if (!isTickResult(result.rows[0]?.result)) throw new SuperadminCalendarCommandError("invalid_response");
      ...
      throw new SuperadminCalendarCommandError(commandCode(error as { code?: string; message?: string }), error);
    } finally {
      client?.release();
      await pool.end();
    }
