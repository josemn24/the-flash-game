...
> E10 ya usa `question-assets` privado; la reutilización en `multiple-choice` sigue pendiente;
...
| Pruebas   | Vitest, type tests, pgTAP, inventario de seguridad, carreras, integración Auth/HTTP/Storage y E2E local para S01–S13, D08a/D08b, E01–E04 y E10.                                                                                                                                    | Imágenes privadas en `multiple-choice` y verificación contra un entorno remoto.                                                                       |
...
| D08 | **Resuelto e implementado en D08a/D08b + S13/E10 (2026-09-18).** `avatars` es público por enlace y `question-assets` privado; se persisten rutas/`assetId`, nunca URLs firmadas. | Imágenes privadas en `multiple-choice`, thumbnails y purga operativa. | Registro `media_assets`, validación server-side de bytes/tipo/dimensiones/hash, subida confirmada, auditoría, compensación y URLs runtime autorizadas; la entrega de E10 no se presenta como protección contra copia. |
...
| D08b             | Integrar `question-assets` privado con el editor superadmin y E10; publicar `assetId` y resolver URL firmada tras `prepare_interaction`. | D08a, S17a, E10. |
...
S01–S13, D08a/D08b, E01–E04 y E10 están cerradas localmente: su entrega cubre login, perfil persistido, lecturas de
