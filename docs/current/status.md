...
migradas usan fixtures y un store mock normalizado. Las slices S01–S08 y la base transversal del
...
superadministración y la creación auditada de salas privadas.
...
| `/admin`                    | Portal privado server-side para superadmins: consulta de contexto y creación inicial de salas. |
...
- La persistencia real verificada cubre el vertical Flash de S01–S08 y el stack local; no hay
  proyecto remoto vinculado.
- El portal privado de `/admin` permite crear salas activas, asignar un owner existente y
  provisionar un grupo inicial opcional. La gestión posterior de miembros, temporadas, editor,
  publicación y calendario aún no están implementados y no forman parte de la UI pública de la beta;
  Storage también sigue pendiente.
...
- `npm test`: 81 archivos de test y 540 tests superados, incluyendo contratos, adaptador y
  componentes de S08.
- `npm run typecheck`, `npm run lint`, `npm run build`, `npm run type-architecture` y
  `npm run docs:check`: correctos.
- `npm run supabase:schema:test`: correcto; inventario, provisioning, S02–S08, ACL del portal,
  idempotencia, rollback y carreras con conexiones PostgreSQL independientes.
