...
migradas usan fixtures y un store mock normalizado. Las slices S01–S07 y la base transversal del
...
- Portal privado mínimo en `/admin`: contexto del operador superadmin y listado de salas activas,
  con RPC y ACL propios. La UI es de solo lectura; sus primeras mutaciones llegarán en S08.
...
habilite. La primera superficie `/admin` ya está disponible únicamente para consultar el contexto
del operador y las salas activas; no ofrece todavía acciones de gestión.
...
| `/admin`                    | Portal privado server-side de solo lectura para superadmins; gestión real pendiente. |
...
- El portal privado mínimo de solo lectura está implementado en `/admin`. La gestión de salas,
  provisionamiento de miembros, configuración de temporadas, editor, publicación y calendario aún
  no están implementados y no forman parte de la UI pública de la beta; Storage también sigue
  pendiente.
...
- `npm run supabase:schema:test`: correcto; inventario, provisioning, S02–S07, portal privado
  (15 checks) y carreras con conexiones PostgreSQL independientes.
