    ...
    <motion.section className="mx-auto min-h-[100dvh] w-full max-w-4xl px-4 py-5 sm:px-6 sm:py-7" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
      ...
      <div className="space-y-3">
        {stage.questions.map((question, index) => {
          ...
          return (
            <motion.details key={question.id} className={`${styles.reviewCard} ${correct ? styles.reviewCorrect : unanswered ? styles.reviewUnanswered : styles.reviewWrong}`} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * 0.035, 0.3) }}>
              <summary>
                ...
                <span className={`${styles.reviewStatus} ${correct ? styles.statusCorrect : unanswered ? styles.statusUnanswered : styles.statusWrong}`}>
              ...
              <div className={styles.reviewContent}>
                {question.type === "logic-code" ? (
                          ...
                          <div className={styles.logicReviewAttempts}>
                            {result.submittedCodes.map((code, attemptIndex) => (
                              <b
                                ...
                                className={attemptIndex === result.submittedCodes!.length - 1 ? styles.logicReviewLast : ""}
                              >
                                {code}
                              </b>
                            ))}
                    ...
                    {question.items.map((item) => {
                      ...
                      return (
                        <div key={item.label} className={styles.classificationReviewRow}>
                          ...
                          <span>
                            <small>Elegida</small>
                            <b className={itemCorrect ? styles.classificationValueCorrect : styles.classificationValueWrong}>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className={styles.answerBox}><span>Tu respuesta</span><strong>{answerLabel(result.answer)}</strong></div>
                    <div className={`${styles.answerBox} ${styles.answerBoxCorrect}`}>
